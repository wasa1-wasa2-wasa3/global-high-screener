...
- 条件: Nasdaq (NMS) または NYSE (NYQ)、時価総額 **$1B 以上 $20B 以下**、時価総額降順
...
| Rule of 40 | ≥ 0（取得できた場合のみ。マイナスは除外） |
| スコア | ≥ 50pt |
- 最終 25 件を残す

#### Phase 4 — Island Reversal 検知 + RS算出
- 最終 25 件に対して v8/chart（1年 OHLC）を並列取得（QQQ 含む）
- アイランド底（gap down → 孤立 → gap up）を検出し `islandReversal: true` を付与
- RS = (銘柄1年リターン倍率) / (QQQ1年リターン倍率) を算出しスコアに反映
...
| 2 | 小型株プレミアム | 25pt | ≤$2B→25、≤$5B→20、≤$10B→14、≤$20B→6 |
...
| 9 | RS vs QQQ | 8pt | ≥3x→8、≥2x→5、≥1.5x→3、≥1x→1 |
| 10 | Rule of 40 | 10pt（マイナスで-5ptペナルティ） | ≥60→10、≥50→7、≥40→5、≥30→2、<0→-5 |

### ポスト補正（加点後に適用）

| ルール | 条件 | 処理 |
|--------|------|------|
| A | ≤$2B 超小型 | × 1.2 マルチプライヤー（**Math.min(100,…) 前**に適用） |
| B | Rev YoY > 50% | 最低 B ランク保証（score ≥ 63） |
| C | Rev≥20% かつ GM≥70% | 最低 B ランク保証（score ≥ 63） |
| G | <$10B かつ Rev>50% | 最低 A ランク保証（score ≥ 70） |
| D | $20B超 | B 以下に制限（score ≤ 55） |
...
| シグナル | 条件 |
|----------|------|
| 🔴 突破 | 価格 ≥ 52W高値×99.9% かつ 出来高比 ≥ 1.5x かつ 前日比 > +0.5% |
...
| ✅ 保有継続 | MA50乖離 0〜+25% かつ 出来高比 ≥ 0.8x かつ 前日比 > -3% |
| 🛑 損切り検討 | MA50乖離 < -15%（$2B超）/ < -25%（$2B以下） |
| ⚡ 超小型 | 時価総額 ≤ $1.5B（スリッページ注意） |
...
> **注意**: `getTrendMode` は購入ボタン表示・メール通知など「アクション」を制御するため、シグナルアイコン（`getSignals`）より閾値が厳しい。

| モード | 条件 |
|--------|------|
| `breakout` | 52W高値の 0.1% 以内 かつ **出来高比 ≥ 2.0x** かつ **前日比 > +1.5%** |
